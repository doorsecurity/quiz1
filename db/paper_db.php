...
function saveQuestions($sessionlist, $branchlist, $semesters, $subjects, $date, $questioncount, $duration, $passcode, $username, $marks, $qids){
    $st = 0;
        $con = getConnection();    

        $con->autocommit(false);
        $statement = $con->prepare("insert into paperdetails (sessionid, branchid, semesterid, subjectid, examdate, questioncount, duration, teacherid, passcode, marksperquest) values (?,?,?,?,?,?,?,?,?,?);");
        $statement->bind_param("ssssssssss",$session_f, $branch_f, $semester_f, $subject_f, $date_f, $questioncount_f, $duration_f, $passcode_f, $teacher_f,$marksperquest_f);
